...
while getopts "p:1:2:t:c:L:d:M:SHA" option; do
	case "${option}" in
		p) PREFIX=${OPTARG};;
		1) READ1=${OPTARG};;
		2) READ2=${OPTARG};;
...
SOAP_NT_IDX=${DB}/refseq/refseq_plus_complete_genome_fungi_and_protozoa_masked
...
ACC2TID=${DB}/tax/abcfhv_complete_genome_fungi_and_protozoa.accession2taxid
